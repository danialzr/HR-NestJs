...
export class PayrollManagerService {
    ...
    async create(dto: CreatePayrollDto) {
        ...
        if (existing) throw new BadRequestException('The payslip for this month has been issued');
    ...
    async findOne(id: number) {
        const payroll = await this.payrollRepo.findOne({ where: { id }, relations: ['user'] })
